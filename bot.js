...
// Pastikan module ada (bisa dihilangkan jika sudah install manual)
...
async function startBot(sessionPhone) {
    ...
    const sock = makeWASocket({
        version,
        logger: pino({ level: "fatal" }), // Level fatal agar log bersih
        printQRInTerminal: false,
        auth: {
            creds: state.creds,
            keys: makeCacheableSignalKeyStore(state.keys, pino({ level: "fatal" }).child({ level: "fatal" })),
        },
        // GANTI BROWSER KE UBUNTU AGAR PAIRING CODE LANCAR
    ...
    if (!sock.authState.creds.registered) {
        // Tunggu 6 detik agar koneksi inisialisasi stabil
        setTimeout(async () => {
            try {
                // Pastikan sessionPhone hanya angka (sesuai nama folder)
                const code = await sock.requestPairingCode(sessionPhone);
                console.log(`KODE PAIRING: ${code}`);
            } catch (err) {
                console.log(`❌ Gagal Pairing: ${err.message}`);
                // Jika gagal karena restart loop, server.js akan menangkap error ini
            }
        }, 6000); 
    }
